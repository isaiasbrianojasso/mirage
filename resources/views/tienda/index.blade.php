...
<div class="header-top">
  ...
  <a href="{{ route('tienda.index') }}">
    <img class="logo img-fluid"
         src="{{ $businessSetting->logo_path ?? '/tienda_assets/img/mirage-logo-1534899548.jpg' }}"
...
<!-- /Block search module TOP -->
        ...
        @auth
            <a href="{{ route('dashboard') }}" title="Mi Cuenta" rel="nofollow" class="header-btn header-user-btn">
                <i class="fa fa-user fa-fw icon" aria-hidden="true"></i>
                <span class="title">Mi Cuenta</span>
            </a>
        @else
            <a href="{{ route('login') }}" title="Acceda a su cuenta de cliente" rel="nofollow" class="header-btn header-user-btn">
                <i class="fa fa-user fa-fw icon" aria-hidden="true"></i>
    ...
    <div id="ps-shoppingcart"
        ...
        <a id="cart-toogle" href="{{ route('cart.index') }}" class="cart-toogle header-btn header-cart-btn">
            <i class="fa fa-shopping-bag fa-fw icon" aria-hidden="true">
                <span class="cart-products-count-btn {{ $cartCount == 0 ? 'd-none' : '' }}" style="display: {{ $cartCount > 0 ? 'inline-block' : 'none' }}; background:red; color:white; border-radius:50%; padding:2px 6px; font-size:10px; position:absolute; top:-5px; right:-5px;">{{ $cartCount }}</span>
            </i>
            <span class="info-wrapper">
            <span class="title">Carrito:</span>
            <span class="cart-toggle-details">
            <span class="text-faded cart-separator"> / </span>
                            {{ $cartCount > 0 ? $cartCount . ' articulos' : 'Vacío' }}
                        </span>
            </span>
        </a>
...
@foreach($categories as $category)
    @if($category->children->count() > 0)
        <li class="cbp-hrmenu-tab cbp-hrmenu-tab-has-sub">
            <a href="{{ route('tienda.category', $category->slug) }}" class="nav-link">
...
<li class="mobile-menu__tab  d-flex flex-wrap js-mobile-menu__tab">
    <a href="{{ route('tienda.category', $category->slug) }}" class="flex-fill mobile-menu__link">{{ $category->name }}</a>
</li>
@endforeach
    ...
    MXN     $    <div class="mobile-menu__language-currency-dropdown">
        <ul>
			...
			<div class="mobile-menu__user">
                @auth
                    <a href="{{ route('dashboard') }}" class="text-reset"><i class="fa fa-user" aria-hidden="true"></i> Mi Cuenta</a>
                @else
                    <a href="{{ route('login') }}" class="text-reset"><i class="fa fa-user" aria-hidden="true"></i> Iniciar sesión</a>
                @endauth
			</div>
...
<!-- /Block search module TOP -->
                ...
                <div class="col col-mobile-btn col-mobile-btn-cart ps-shoppingcart text-center">
                   <div id="mobile-cart-wrapper">
                    ...
                    @endphp
                    <a href="{{ route('cart.index') }}" class="m-nav-btn"><i class="fa fa-shopping-bag mobile-bag-icon" aria-hidden="true">
                        <span id="mobile-cart-products-count" class="cart-products-count cart-products-count-btn {{ $cartCount == 0 ? 'd-none' : '' }}" style="display: {{ $cartCount > 0 ? 'inline-block' : 'none' }}; background:red; color:white;">
                                                                    {{ $cartCount }}
                            </span></i>
                        <span>Carrito</span></a>
                   </div>
  ...
  <section id="main">
    ...
    <div class="swiper-slide">
        <div class="swiper-slide-inner">
            ...
            <img class="swiper-slide-image swiper-lazy" 
                 width="100%" height="auto" 
                 src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 6243 1961 '%3E%3C/svg%3E" 
                ...
                <div class="thumbnail-container">
                    <a href="{{ route('tienda.product', $product->slug) }}" class="thumbnail product-thumbnail">
                        @if($product->images->isNotEmpty())
                            <img
                                src="{{ $product->images->where('is_primary', true)->first()->image_url ?? $product->images->first()->image_url }}"
                                alt="{{ $product->name }}"
                ...
                <div class="product-description">
                    ...
                    <div class="product-add-cart">
                        <form action="{{ route('cart.add', $product->id) }}" method="POST">
                            @csrf
                            <button class="btn btn-primary btn-sm add-to-cart" data-button-action="add-to-cart" type="submit">
