...
<div class="header-top">
  ...
  <a href="{{ route('tienda.index') }}">
    <img class="logo img-fluid"
         src="{{ $businessSetting->logo_path ?? '/tienda/img/mirage-logo-1534899548.jpg' }}"
...
<div class="container iqit-megamenu-container"><div id="iqitmegamenu-wrapper" class="iqitmegamenu-wrapper iqitmegamenu-all">
    ...
    @if($cat->children->count() > 0)
        <li class="cbp-hrmenu-tab cbp-hrmenu-tab-has-sub {{ $category->slug === $cat->slug ? 'current' : '' }}">
                ...
                <div class="cbp-hrsub-inner">
                    <ul>
                        @foreach($cat->children as $child)
                            <li>
                                <a href="{{ route('tienda.category', $child->slug) }}" class="{{ $category->slug === $child->slug ? 'current' : '' }}">{{ $child->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
    ...
    @else
        <li class="cbp-hrmenu-tab {{ $category->slug === $cat->slug ? 'current' : '' }}">
            <a href="{{ route('tienda.category', $cat->slug) }}" class="nav-link">
                <span class="cbp-tab-title">{{ mb_strtoupper($cat->name) }}</span>
            </a>
        </li>
...
<li class="mobile-menu__tab  d-flex flex-wrap js-mobile-menu__tab">
    <a href="{{ route('tienda.category', $cat->slug) }}" class="flex-fill mobile-menu__link">{{ $cat->name }}</a>
</li>
@endforeach
...
<div class="mobile-menu__currency-selector d-inline-block">
    MXN     $    <div class="mobile-menu__language-currency-dropdown">
        <ul>
			...
			<a href="{{ auth()->check() ? route('dashboard') : route('login') }}" class="text-reset"><i class="fa fa-user" aria-hidden="true"></i>
...
<!-- /Block search module TOP -->
                ...
                <div class="col col-mobile-btn col-mobile-btn-account text-center">
                    <a href="{{ auth()->check() ? route('dashboard') : route('login') }}" class="m-nav-btn"><i class="fa fa-user" aria-hidden="true"></i>
                        <span>     
                                                            Iniciar sesión
                                                        </span></a>
                </div>
...
<nav data-depth="2" class="breadcrumb">
            <div class="container">
                ...
                <div class="col">
                    <ol>
                                                                            ...
                                                                            <li>
                                            <a href="{{ route('tienda.index') }}"><span>Inicio</span></a>
                                        </li>
 ...
 <div id="content-wrapper"
    ...
    <div class="row align-items-center justify-content-between small-gutters">
            ...
            <a href="{{ request()->url() }}?order=product.position.asc&productListView=grid" class="current js-search-link hidden-sm-down" data-button-action="change-list-view" data-view="grid"  rel="nofollow"><i class="fa fa-th" aria-hidden="true"></i></a>
            <a href="{{ request()->url() }}?order=product.position.asc&productListView=list" class=" js-search-link hidden-sm-down" data-button-action="change-list-view" data-view="list"  rel="nofollow"><i class="fa fa-th-list" aria-hidden="true"></i></a>
        </div>
...
<div class="products-sort-nb-dropdown products-sort-order dropdown">
    ...
    <div class="dropdown-menu">
                                                            <a
                    rel="nofollow"
                    href="{{ request()->url() }}?order=product.position.asc"
            ...
            >
                Relevancia
                                ...
                                <a
                    rel="nofollow"
                    href="{{ request()->url() }}?order=product.name.asc"
            ...
            >
                Nombre, A a la Z
                                ...
                                <a
                    rel="nofollow"
                    href="{{ request()->url() }}?order=product.name.desc"
                    class="select-list dropdown-item js-search-link"
            >
                Nombre, Z a la A
            </a>
                                <a
                    rel="nofollow"
                    href="{{ request()->url() }}?order=product.price.asc"
                    class="select-list dropdown-item js-search-link"
            >
                Menor al mayor precio
                                ...
                                <a
                    rel="nofollow"
                    href="{{ request()->url() }}?order=product.price.desc"
                    class="select-list dropdown-item js-search-link"
            >
        ...
        <article class="product-miniature product-miniature-default product-miniature-grid product-miniature-layout-1 js-product-miniature">
            <div class="thumbnail-container">
                <a href="{{ route('tienda.product', $product->slug) }}" class="thumbnail product-thumbnail">
                    ...
                    @endphp
                    @if($primaryImage)
                        <img src="{{ Str::startsWith($primaryImage->image_url, 'http') ? $primaryImage->image_url : Storage::url($primaryImage->image_url) }}" alt="{{ $product->name }}" class="img-fluid js-lazy-product-image lazy-product-image product-thumbnail-first" style="width:236px;height:305px;object-fit:contain;">
                    @else
                        <img src="data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%20236%20305'%3E%3C/svg%3E" class="img-fluid js-lazy-product-image lazy-product-image product-thumbnail-first">
                    @endif
                </a>
            ...
            <div class="product-description">
                ...
                <div class="product-reference text-muted"> 
                    <a href="{{ route('tienda.product', $product->slug) }}">{{ $product->sku }}</a>
                </div>    
                @endif
            ...
            <div class="product-add-cart">
                <form action="{{ route('cart.add') }}" method="post">
                    ...
                    <div class="input-group-add-cart">
                        <input type="number" name="quantity" value="1" class="form-control input-qty" min="1">
                        <button class="btn btn-product-list add-to-cart" type="submit">
...
<nav class="pagination">
  ...
  <ul class="category-sub-menu">
      @foreach($categories as $cat)
          <li data-depth="0"><a href="{{ route('tienda.category', $cat->slug) }}">{{ $cat->name }}</a></li>
      @endforeach
  </ul>
    </div>
  </div>
			...
			<h4 class="title_block">Your account</h4>
				...
				<section class="login-form">
					<form id="login-form" action="{{ route('login') }}" method="post">
							...
							<a class="a_link_17 pull-right" href="{{ route('password.request') }}" title="Recover your forgotten password" rel="nofollow">Forgot your password?</a>
                ...
                <ul>
                                ...
                                <a
                                    ...
                                    Mi cuenta
                                                                                                ...
                                                                                                <li>
                                <a
                                        href="{{ auth()->check() ? route('customer.orders') : route('login') }}"
                                        title=""                                                                        >
                                    Historial de pedidos
                                                                                                ...
                                                                                                <li>
                                <a
                                        href="{{ auth()->check() ? route('profile.show') : route('login') }}"
                                        title=""                                                                        >
                                    Identidad
                                </a>
                            </li>
...
<div id="iqitwishlist-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <form  id="login-form-modal" action="{{ route('login') }}"   method="post">
